...
function normalizeVariant(
  ...
  const availableStock = Number(
    v.availableStock ?? Math.max(0, stock - reserved),
  );
  const status: VariantStatus = v.status === "ACTIVE" ? "ACTIVE" : "INACTIVE";
  const isPurchasable =
    productStatus === "ACTIVE" &&
    (typeof v.isPurchasable === "boolean"
      ? v.isPurchasable
      : status === "ACTIVE" && availableStock > 0);
  return {
    id: v.id,
    sku: v.sku,
    name: v.name,
...
export function VariantManager({
  ...
  const [variants, setVariants] = useState<AdminVariant[]>(() =>
    initialVariants.map((variant) => normalizeVariant(variant, productStatus)),
  );
  ...
  function openDelete(variant: AdminVariant) {
    setError("");
  ...
  /** CRUD 接口返回的可售状态由服务端计算，商品不是 ACTIVE 时前端强制不可售。 */
  function normalizeFromApi(v: AdminVariant): AdminVariant {
    return {
      ...v,
      isPurchasable: productStatus === "ACTIVE" && v.isPurchasable,
    };
  }
  ...
  const columns: Column<AdminVariant>[] = [
    ...
    },
    {
      ...
      header: "状态",
      render: (row) => {
        const isPurchasable = productStatus === "ACTIVE" && row.isPurchasable;
        if (isPurchasable) {
          return <StatusBadge status="active" label="可售" />;
        }
        if (productStatus === "DRAFT") {
          return <StatusBadge status="pending" label="商品草稿" />;
        }
        if (productStatus === "ARCHIVED") {
          return <StatusBadge status="inactive" label="商品已下架" />;
        }
        return (
          <StatusBadge
  ...
  return (
    <section className="variant-manager" aria-labelledby="variant-manager-title">
      ...
      <VariantFormModal
        key={
          editing ? `edit-${editing.id}-${formSession}` : `create-${formSession}`
        }
        open={formOpen}
      ...
      >
        <p>
          ...
          {deleteTarget?.name}）吗？若该 SKU 已被购物车或订单引用，服务端会拒绝删除并返回
          409；请先编辑 SKU 并将其停用。
        </p>
        {error ? <p className="form-error">{error}</p> : null}
      </Modal>
