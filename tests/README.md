...
- 真实 SKU 加购、累计加购、修改数量和库存不足；
- 下单前二次库存校验、服务端价格重算、订单金额与名称/价格快照；
- 购物车下单后清空、订单归属隔离、Admin 订单查询与状态流转；
- 空购物车与空购物车下单；
- 经销商申请、管理员审批、角色更新和企业数据隔离。

完整“选择 SKU -> 加入购物车 -> 创建订单”场景已经启用。测试通过 Admin API 创建本轮专用 SKU，并主动改变库存和价格，证明购物车与下单均重新读取服务端数据，而不是使用客户端价格或零价格占位。P1 只要求下单前二次校验库存，不执行库存扣减或预占。
...
不要把本测试指向共享开发、演示或生产环境；测试会注册临时用户，并创建专用 SKU、购物车、订单、经销商申请和企业记录。
